...
Development of so-called ``hereditary stratigraphy'' methodology addressing this problem poses an even more general problem: how to satisfactorily maintain a temporally representative collection of observations that stream in on a rolling basis.
We term this more general question the ``stream curation problem.''
Because ongoing data feeds necessitate continual downsampling when faced with storage and memory limitations, the stream curation problem occurs also in context of data streams, and relates closely to the concept of binning in this context.
To meet requirements of diverse stream curation scenarios, we present a suite of stream curation policy algorithms that trade off along a continuum of collection size and temporal coverage.
...
Policy algorithm designs pre-determine retained collection composition at each time point in a computationally reducible manner, enabling key optimizations to reduce storage overhead and streamline processing of incoming observations.

Hereditary stratigraphy's application of stream curation to lineage inference arises through interpretation of each individual as an independent observation of lineage association within an ongoing generational stream.
...
New checkpoints are created each generation, and stream curation policy algorithms decide which to retain.
Recent experimental work applying this principle has yielded robust recovery of phylogenetic information.
Here, we formalize the theoretical foundations of hereditary stratigraphy through exposition of stream curation algorithms and describe a procedure to perform ancestry tree reconstruction from a population of hereditary stratigraph annotations.
...
% Implications of underlying stream curation algorithms extend more broadly still, encompassing wider issues of rolling record management.
\end{abstract}
